...
def test_calculate_timelag_steplength_speed_raises_if_data_is_not_geodataframe():
    with pytest.raises(TypeError):
        calculate_timelag_steplength_speed(data="not a geodataframe", datetime_col="time", geometry_col="geometry")

def test_calculate_timelag_steplength_speed_raises_if_datetime_col_is_not_string():
    gdf = gpd.GeoDataFrame()
    with pytest.raises(TypeError):
        calculate_timelag_steplength_speed(data=gdf, datetime_col=123, geometry_col="geometry")

def test_calculate_timelag_steplength_speed_raises_if_geometry_col_is_not_string():
    gdf = gpd.GeoDataFrame()
    with pytest.raises(TypeError):
        calculate_timelag_steplength_speed(data=gdf, datetime_col="time", geometry_col=123)